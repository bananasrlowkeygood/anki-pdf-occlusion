...
| `editing_mask_opacity` | int 0–255 | `150` | Opacity of the mask rectangles **in the editing canvas**, before any card is made. `150` (the default) keeps the slide readable while you draw. Raise it to `255` and each box hides what it covers the moment it is drawn, so building the deck is itself a recall exercise. Press **P** (or the Peek button) at any time to see through the masks without changing this setting. Does not affect the cards — that is `mask_opacity`. |
